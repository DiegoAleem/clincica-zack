...
export class CandidatoService {
  ...
  getCandidatos(pagina: number, filtro: string, campoOrdenado: string, ordemAscendente: string): Observable<any> {
    ...
    return this.http.get<any[]>('https://92.112.177.53:8443/candidato/filtrados', {headers: this.getHeader(), params: params}).pipe(
  ...
  excluirCandidato(id: number):  Observable<any> {
    ...
    return this.http.delete<any>(`https://92.112.177.53:8443/candidato/excluir/${id}`, {headers:this.getHeader()}).pipe(
      catchError((error: any) => this.handleError(error)) 
    );
  }
